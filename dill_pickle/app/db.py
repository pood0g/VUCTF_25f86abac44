...
class Database:
    ...
    def search_user(self, table: str, return_columns: str, where_like: tuple = None):
        ...
        if where_like:  # if parameter passed execute this
            sql_statement += f" WHERE ? like ?;"
        try:
            rows = self.__cursor(sql_statement, (where_like[0], f"%{where_like[1]}%"), fetch=True)
            return rows
        except sqlite3.Error as error:
            print(" !", str(error).capitalize())

    def login(self, creds: tuple):
        sql_statement = "SELECT * FROM users WHERE username = ? and password = ?"
        try:
            row = self.__cursor(sql_statement, creds, fetch=True)
            if row:
                return row[0]
        except sqlite3.Error as error:
            print(" !", str(error).capitalize())
